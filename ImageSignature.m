...
- (Status)checkAuthorizationStatusForBase64:(BOOL)base64
                          withRequestedSize:(CGFloat)requestedSize
                                andDelegate:(__nullable id<ImageSignatureDelegate>)delegate
{
...
- (NSData*)saveImage:(nonnull UIImage*)image compressRate:(int)compressRate{
    ...
    if([fileManager fileExistsAtPath:_localId]){
        NSLog(@"ImageSignature - File already exists. Suppressing it... %@",_localId);
    ...
    if(error){
        NSLog(@"ImageSignature - Error writing: %@",error);
    }
    else {
        NSLog(@"ImageSignature - Save completed at %@",_localId);
    }
    return imageData;
}
